...
/* ******* MODIFIED ShanChenMultiComponentProcessor3D by WINDOWKIM ******* */
/* *************** ShanChenPsiMultiComponentProcessor3D ***************** */

template<typename T, template<typename U> class Descriptor>
ShanChenPsiMultiComponentProcessor3D <T,Descriptor>::ShanChenPsiMultiComponentProcessor3D(T G_)
    : G(G_)
{ }

template<typename T, template<typename U> class Descriptor>
ShanChenPsiMultiComponentProcessor3D <T,Descriptor>::ShanChenPsiMultiComponentProcessor3D(
        std::vector<std::vector<T> > const& speciesG_)
    : G((T) 0)
{
    pluint numSpecies = speciesG_.size();
...
template<typename T, template<typename U> class Descriptor>
void ShanChenPsiMultiComponentProcessor3D<T,Descriptor>::process (
        Box3D domain,
        std::vector<BlockLattice3D<T,Descriptor>*> lattices )
{
    // Number of species (or components) which are coupled in this Shan/Chen multi-component fluid.
...
template<typename T, template<typename U> class Descriptor>
ShanChenPsiMultiComponentProcessor3D<T,Descriptor>*
    ShanChenPsiMultiComponentProcessor3D<T,Descriptor>::clone() const
{
    return new ShanChenPsiMultiComponentProcessor3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void ShanChenPsiMultiComponentProcessor3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    // All blocks are modified by the Shan/Chen processor.
    for (pluint iBlock=0; iBlock<modified.size(); ++iBlock) {
        modified[iBlock] = modif::staticVariables;
